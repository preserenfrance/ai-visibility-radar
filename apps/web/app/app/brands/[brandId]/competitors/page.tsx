...
export default async function CompetitorsPage({
  ...
  const totalMentions = mentionGroups.reduce(
            ...
            <TBody>
              {competitors.map((competitor) => {
                const group = mentionGroups.find(
                  (item) => item.entityName === competitor.name,
                );
                const runs = losingRuns.filter((run) =>
                  run.aiResponse?.mentions.some(
                    (mention) => mention.entityName === competitor.name,
                  ),
                );
                const citations = [
                  ...new Set(
                    ...
                    <TD>
                      <details>
                        ...
                        <form
                          ...
                          className="mt-3 grid min-w-72 gap-3 rounded-md border bg-secondary/30 p-3"
                        >
                          <input
                            type="hidden"
                            name="competitorId"
...
function sentimentForRuns(runs: Array<any>) {
  const sentiments = runs.flatMap(
    (run) =>
      run.aiResponse?.mentions.map((mention: any) => mention.sentiment) ?? [],
  );
