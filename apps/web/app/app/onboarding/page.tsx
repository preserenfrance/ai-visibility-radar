...
import { Globe, ListChecks, ScanSearch } from "lucide-react";
...
async function onboard(formData: FormData) {
  ...
  await generatePromptsForBrand(brand.id, 25).catch((error) => {
    console.error("Onboarding prompt generation failed", error);
    return null;
  });
    ...
    <section className="mx-auto grid max-w-7xl gap-6 px-5 py-8 lg:grid-cols-[0.85fr_1.15fr]">
      <div>
        ...
        <p className="mt-3 max-w-xl text-muted-foreground">
          V enem toku ustvari organizacijo, znamko, konkurente, crawl posnetek in začetni set promptov.
        </p>
      ...
      <Card>
        ...
        <OnboardingBrandForm action={onboard} />
      </Card>
    </section>
  );
}
